...
export default function Layout({ children }) {
  ...
  const visiblePages = ["/haqqimizda", "/elaqe"]; // Add the paths of the pages where the element should be visible

  const isElementInvisible = !visiblePages.includes(router.pathname);
      ...
      <div className="flex justify-center">
        {isElementInvisible && (
          <div className="z-90 flex justify-center items-center lg:hidden">
            <SifarishBtn classNames="flex bottom-[62px] fixed" />
          </div>
        )}
      </div>
