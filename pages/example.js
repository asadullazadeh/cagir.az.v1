...
const DropDownPage = () => {
  ...
  const handleSelectionChange = (dropdownIndex, selectedValue) => {
    // Create a new path based on the selected dropdown value
    const newPath = `/sifaris/${selectedValue}`;
    // const newPath = `/sifaris/${dropdownIndex}/${selectedValue}`;

    // Use Next.js Router to navigate to the new path
    router.push(newPath);
  };

  return (
    <div>
      {/* Replace these dropdowns with your actual dropdown components */}
      <select onChange={(e) => handleSelectionChange(1, e.target.value)}>
        <option value="">Select option 1</option>
        <option value="temizlik-xidmeti">Təmizlik xidməti</option>
        <option value="usta">Usta xidməti</option>
      </select>

      <select onChange={(e) => handleSelectionChange(2, e.target.value)}>
        <option value="">Select option 2</option>
        <option value="option3">Option 3</option>
        <option value="option4">Option 4</option>
      </select>

      <select onChange={(e) => handleSelectionChange(3, e.target.value)}>
