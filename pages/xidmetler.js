      ...
      <div className="flex flex-col sm:pt-[36px] md:pt-[42px] lg:pt-[48px] xl:pt-[54px] 2xl:pt-[60px] py-[60px] sm:py-[75px] md:py-[90px] lg:py-[105px] xl:py-[120px] 2xl:py-[135px]">
        ...
        <ul className="grid grid-cols-2 lg:grid-cols-3 gap-[10px] lg:gap-[60px] px-[10px] justify-between">
          {responseData.map(({ id, imageUrl, nameUrl, serviceNames }) => (
            <li key={id}>
              <Link href={nameUrl} className="block rounded-[10px] lg:rounded-[25px] bg-white p-[11px] lg:p-[26px] group hover:drop-shadow-card transition duration-300">
                <div className="relative mb-[5px] lg:mb-[15px]">
                  <Image width={367} height={283} src={`https://api.cagir.az${imageUrl}`} alt={nameUrl} className="rounded-[5px] sm:rounded-[20px] w-full aspect-[123/96] object-cover object-center" />
                </div>
                {serviceNames.map((item, index) => (
                  <div key={index} className="flex items-center justify-between">
                    <h5 className="font-bold text-[14px] lg:text-[20px] leading-[21px] lg:leading-[30px] text-black500">{item.name}</h5>
